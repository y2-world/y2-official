...
// <input list="..."> のdatalistはSafari/iOS等のモバイルブラウザで候補が
// 出ないことがあるため、自前のドロップダウン候補リストに置き換える。
// 呼び出し側は <input data-autocomplete-options='["曲名1","曲名2",...]'> を
// 用意しておけば、ページ内のどのタイミングで動的に追加された要素でも
// initAutocomplete(input) を呼ぶだけで候補表示が有効になる。
function initAutocomplete(input) {
    ...
    let options = [];
    try {
        options = JSON.parse(input.dataset.autocompleteOptions || '[]');
    } catch (e) {
        options = [];
    }
    ...
    list.className = 'app-autocomplete-list';
    list.hidden = true;

    // inputは position: relative なラッパーが無い前提でも使えるよう、
    // ドロップダウンはinputの直後に挿入してJSで位置合わせする。
    ...
    function renderMatches() {
        ...
        matches.forEach((match) => {
            ...
            item.className = 'app-autocomplete-item';
            item.textContent = match;
            item.addEventListener('mousedown', (e) => {
                // blurより先にmousedownが発火するので、blurでリストが消える前に確定できる
                e.preventDefault();
                input.value = match;
                list.hidden = true;
    ...
    function setActive(index) {
        const items = list.querySelectorAll('.app-autocomplete-item');
